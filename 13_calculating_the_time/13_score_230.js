...
function calculateTime(deliveries) {
  ...
  const limit = 7 * 3600;
  const mytime = hhmmss[0] * 3600 + hhmmss[1] * 60 + hhmmss[2];

  const today = new Date();
  today.setHours(0); // gmt - 5
  today.setMinutes(0);
  today.setSeconds(0);

  today.setSeconds(Math.abs(mytime - limit));
  return (mytime < limit ? "-" : "") + today.toISOString().substring(11, 19);
}
